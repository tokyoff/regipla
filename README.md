...
仕様をオープンにすることで、「レジプラ」を利用したアプリの開発を自由に行ってもらうことを目的としています。これからは、煩わしいタップ操作にとらわれることなく、押しやすい物理ボタンを使った快適なアプリ操作が可能になるのです。
...
[公式ストア](https://flipflop.booth.pm/)では、レジプラ以外にも「定礎シール」や「おっさん写植Tシャツ」など、愉快なグッズがいっぱいありますので、是非ついでにご覧になって下さい。レジプラについての最新情報は、[Twitter](https://twitter.com/tokyo_ff)や公式サイト [レジプラ.com](http://xn--yck2c9bj.com/) にてお知らせしています。



## アプリ開発方法

レジプラは、BLE準拠のペリフェラルデバイスとして動作します。ご自身のアプリ上で、BLEの通信を行うプログラムを書いていただく必要があります。

レジプラ用のライブラリなどは、現時点ではありません。各OSが提供するBLEのライブラリを用いて、以下に記載したプロトコルの実装をお願いします。
...
※ koshianのPIO5は基板上には出ていませんが、直接モジュールにハンダ付けをすれば、汎用デジタル入出力ポートとして使用可能です。
...
- Property: Write Without Response
- 1をwriteすると、ソフトウェアリセットがかかります

### Low Battery Notification

- Property: Read, Notify
- バッテリ残量が10％未満 かつ いずれかのボタンを押した（PIO割込み発生）タイミングで、1を通知します
...
使用に際しては、PIO関係のUUIDをレジプラ仕様に合わせて変更する必要があります。また、PIOのなかでも以下のAPIは正しく動作しません。
...
※ konashiライブラリに関する質問等にはお答えできませんのでご了承下さい。
